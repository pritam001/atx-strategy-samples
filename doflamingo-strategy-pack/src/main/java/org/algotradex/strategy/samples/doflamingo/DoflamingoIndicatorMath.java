...
/**
 * Package-local indicator implementation used by the Doflamingo sample ports.
 * <p>
 * The calculations mirror the sample strategy requirements closely enough for deterministic replay
 * and tests. They are not a shared platform indicator library, do not load market data, and do not
 * guarantee exchange/vendor parity beyond the formulas encoded here.
 */
final class DoflamingoIndicatorMath {
    ...
    private static MacdSeries laggedMacd(List<BarEvent> bars, int fastPeriod, int slowPeriod, int signalPeriod) {
        ...
        double[] laggedMacd = lagged(macd);
        double[] laggedSignal = lagged(signal);
        double[] histogram = new double[closes.length];
        Arrays.fill(histogram, Double.NaN);
        for (int index = 0; index < closes.length; index++) {
            if (!Double.isNaN(laggedMacd[index]) && !Double.isNaN(laggedSignal[index])) {
                histogram[index] = laggedMacd[index] - laggedSignal[index];
            }
        }
        return new MacdSeries(laggedSignal, histogram);
    }
    ...
    private static StochRsiSeries laggedStochRsi(List<BarEvent> bars, int rsiPeriod, int stochasticPeriod,
        ...
        return new StochRsiSeries(lagged(smaSeries(stochRsi, kPeriod)), lagged(smaSeries(smaSeries(stochRsi, kPeriod), dPeriod)));
    }
    ...
    static final class MultiIndicatorTracker {
        ...
        private int index = -1;
        private RollingIndicators.MacdPoint previousRawMacd;
        ...
        private void advanceMacd(BarEvent bar) {
            ...
            if (previousRawMacd != null) {
                currentSignal = previousRawMacd.signal();
                currentHistogram = previousRawMacd.macd() - previousRawMacd.signal();
            }
            ...
            macd.current().ifPresent(point -> previousRawMacd = point);
        }
    }
    ...
    private static final class LaggedStochRsi {
        ...
        private double lastRawK = Double.NaN;
        private double lastRawD = Double.NaN;
        ...
        private StochRsiPoint update(int index, double close) {
            ...
            k = lastRawK;
            d = lastRawD;
            lastRawK = rawK.orElse(Double.NaN);
            lastRawD = rawD.orElse(Double.NaN);
