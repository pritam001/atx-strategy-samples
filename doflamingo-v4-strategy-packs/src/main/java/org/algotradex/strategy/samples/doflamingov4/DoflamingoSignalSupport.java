...
/**
 * Internal factory for Doflamingo v4 signal and lifecycle-intent records.
 * <p>
...
final class DoflamingoSignalSupport {
    ...
    static TradeSignal shortSignal(String strategyId, String strategyVersion, StrategyExecutionContext context,
                                   BigDecimal confidence, SetupType setupType) {
        return signal(strategyId, strategyVersion, context, confidence, setupType, Direction.SHORT, "short");
    }

    private static TradeSignal signal(String strategyId, String strategyVersion, StrategyExecutionContext context,
                                      BigDecimal confidence, SetupType setupType, Direction direction, String idSide) {
        BarEvent bar = context.currentBar();
        return new TradeSignal(
                strategyVersion,
                new SignalId("signal-" + strategyId + "-" + idSide + "-" + bar.eventId().value().toLowerCase(Locale.ROOT)),
                bar.instrument(),
                direction,
    ...
    static StrategyTradeIntent shortEntryIntent(
            String strategyId,
            String strategyVersion,
            StrategyExecutionContext context,
            BigDecimal confidence,
        ...
        return intent(
                ...
                evidence,
                tags,
                "short-entry-" + bar.eventId().value().toLowerCase(Locale.ROOT),
                conditions
        );
    }
    ...
    static StrategyTradeIntent shortExitIntent(
            String strategyId,
            String strategyVersion,
            StrategyExecutionContext context,
            BigDecimal confidence,
        ...
        return intent(
                ...
                strategyVersion,
                context,
                StrategyTradeAction.EXIT_SHORT,
                PositionSide.SHORT,
                LifecycleRole.EXIT,
                setupType,
                confidence,
                ...
                evidence,
                tags,
                "short-exit-" + bar.eventId().value().toLowerCase(Locale.ROOT),
                conditions
        );
    }
    ...
    static StrategyTradeIntent shortScaleOutIntent(
            String strategyId,
            String strategyVersion,
            StrategyExecutionContext context,
            BigDecimal confidence,
        ...
        return intent(
                ...
                strategyVersion,
                context,
                StrategyTradeAction.SCALE_OUT_SHORT,
                PositionSide.SHORT,
                LifecycleRole.SCALE_OUT,
                setupType,
                confidence,
                ...
                evidence,
                tags,
                "short-scale-out-" + bar.eventId().value().toLowerCase(Locale.ROOT),
                conditions
        );
    }
    ...
    static TradeIntentExitPolicy percentStopWithRrTarget(
        ...
        TradeIntentExitRule target = targetRMultiple == null || targetRMultiple.signum() <= 0
                ? TradeIntentExitRule.none()
                : new TradeIntentExitRule(
                StrategyExitRuleType.RR,
                targetRMultiple.setScale(4, RoundingMode.HALF_UP),
                "Target expressed as reward-to-risk multiple"
        );
        return new TradeIntentExitPolicy(
    ...
    private static StrategyTradeIntent intent(
        ...
        return new StrategyTradeIntent(
                ...
                new StrategyTradeIntentReason(
                        reason == null || reason.isBlank() ? "Doflamingo v4 lifecycle rule" : reason,
                        evidence,
                        tags,
                        conditions),
                new SourceRef(SourceType.STRATEGY, strategyId),
