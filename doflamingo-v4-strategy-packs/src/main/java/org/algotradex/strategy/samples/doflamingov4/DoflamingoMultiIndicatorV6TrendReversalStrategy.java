...
public final class DoflamingoMultiIndicatorV6TrendReversalStrategy implements TradeIntentStrategy {
    ...
    private final double psarMinDistanceLongPct;
    private final int requireRsiExtremeWithinBars;
    private final BigDecimal targetRMultiple;
    private final boolean sessionGating;
    private final BigDecimal maxPortfolioDrawdownPct;
    ...
    DoflamingoMultiIndicatorV6TrendReversalStrategy(BigDecimal minConfidence, int macdFastPeriod, int macdSlowPeriod,
                                                    ...
                                                    int staleBars, BigDecimal staleMinR, int cooldownBars,
                                                    int structureExitConfirmBars, BigDecimal volumeConfirmMultiple,
                                                    BigDecimal psarMinDistanceLongPct, int requireRsiExtremeWithinBars,
                                                    BigDecimal targetRMultiple, boolean sessionGating,
                                                    BigDecimal maxPortfolioDrawdownPct, boolean enableScaleOut,
        ...
        this.psarMinDistanceLongPct = requireNonNull(psarMinDistanceLongPct, "psarMinDistanceLongPct").doubleValue();
        this.requireRsiExtremeWithinBars = Math.max(0, requireRsiExtremeWithinBars);
        this.targetRMultiple = requireNonNull(targetRMultiple, "targetRMultiple");
        this.sessionGating = sessionGating;
        this.maxPortfolioDrawdownPct = requireNonNull(maxPortfolioDrawdownPct, "maxPortfolioDrawdownPct");
    ...
    public StrategyIntentResult onBarIntent(StrategyExecutionContext context) {
        ...
        if (portfolioDrawdownBlocked(context)) {
            return StrategyIntentResult.empty();
        }
        ...
        int momentumBreadth = confirmations(macdHistogramRising, stochKRising, stochDRising);
        boolean momentumBreadthOk = momentumBreadth >= 2;
        boolean rsiExtremeRecent = requireRsiExtremeWithinBars == 0
        ...
        double longPsarDistancePct = percentDistance(current.ohlcv().low().doubleValue(), state.psar(), close);
        boolean psarDistanceOk = longPsarDistancePct >= psarMinDistanceLongPct;
        boolean momentumConfirmed = originalMomentum && momentumBreadthOk;
        ...
        List<StrategyTradeIntentConditionEvidence> conditions = List.of(
                DoflamingoSignalSupport.condition("multi-v6-v4.psar-direction-up", "PSAR confirms upward direction", "PSAR", state.psar(), "<", "Candle low", current.ohlcv().low().doubleValue(), directionNowUp),
                DoflamingoSignalSupport.condition("multi-v6-v4.buy-macd-confirmed", "MACD buy confirmation", "MACD buy pattern", buySignalMacd ? 1.0d : 0.0d, "=", "Required", 1.0d, buySignalMacd),
                DoflamingoSignalSupport.condition("multi-v6-v4.buy-stoch-confirmed", "Stoch RSI buy confirmation", "Stoch buy pattern", buySignalStoch ? 1.0d : 0.0d, "=", "Required", 1.0d, buySignalStoch),
                DoflamingoSignalSupport.condition("multi-v6-v4.original-momentum-confirmed", "Original reversal momentum confirmed", "Original momentum", originalMomentum ? 1.0d : 0.0d, "=", "Required", 1.0d, originalMomentum),
                DoflamingoSignalSupport.condition("multi-v6-v4.macd-histogram-rising", "MACD histogram is improving", "MACD histogram", state.macdHistogram(), ">", "Previous MACD histogram", state.previousMacdHistogram(), macdHistogramRising),
                DoflamingoSignalSupport.condition("multi-v6-v4.stoch-k-rising", "Stoch RSI K is improving", "Stoch K", state.stochK(), ">", "Previous Stoch K", state.previousStochK(), stochKRising),
                DoflamingoSignalSupport.condition("multi-v6-v4.momentum-breadth", "At least two momentum signals are rising", "Rising momentum count", momentumBreadth, ">=", "Required", 2, momentumBreadthOk),
                DoflamingoSignalSupport.condition("multi-v6-v4.rsi-extreme-recent", "RSI recently reached an oversold extreme", "RSI extreme recent", rsiExtremeRecent ? 1.0d : 0.0d, "=", "Required", 1.0d, rsiExtremeRecent),
                DoflamingoSignalSupport.condition("multi-v6-v4.volume-confirmed", "Current volume confirms the reversal", "Volume confirmed", volumeOk ? 1.0d : 0.0d, "=", "Required", 1.0d, volumeOk),
                DoflamingoSignalSupport.condition("multi-v6-v4.long-psar-distance", "PSAR distance avoids noisy long flips", "PSAR distance %", longPsarDistancePct, ">=", "Minimum distance %", psarMinDistanceLongPct, psarDistanceOk),
                DoflamingoSignalSupport.condition("multi-v6-v4.close-above-span-b", "Close above present Span B", "Candle close", close, ">", "Ichimoku Span B", state.presentSpanB(), close > state.presentSpanB()),
                DoflamingoSignalSupport.condition("multi-v6-v4.trend-filter", "Adaptive trend filter passed", "Trend filter", trendFilterPassed ? 1.0d : 0.0d, "=", "Required", 1.0d, trendFilterPassed),
                DoflamingoSessionGate.allowedCondition("multi-v6-v4.session-allowed", context, sessionGating),
                DoflamingoMarketRegimeFilter.allowedCondition("multi-v6-v4.market-regime-allowed", context, skipMarketRegimes),
                DoflamingoSignalSupport.condition("multi-v6-v4.confidence-threshold", "Dynamic confidence meets threshold", "Confidence", confidence.doubleValue(), ">=", "Minimum confidence", minConfidence.doubleValue(), true)
        );
        ...
        StrategyTradeIntent intent = DoflamingoSignalSupport.longEntryIntent(
                ...
                maxHoldingBars,
                "Doflamingo Multi V6 v4 strict PSAR, momentum, cloud, and trend-filter reversal entry",
                List.of(
                ...
                List.of("doflamingo", "v4", "strict", "multi-v6", "entry", "risk", "confidence"),
                conditions
        );
    ...
    private StrategyIntentResult positionIntent(
        ...
        if (canScaleOut) {
            ...
            StrategyTradeIntent scaleIntent = DoflamingoSignalSupport.scaleOutIntent(
                    ...
                    SetupType.REVERSAL,
                    scaleOutFraction,
                    "Doflamingo Multi V6 v4 scale-out at configured R multiple",
                    List.of("currentR=" + currentR, "scaleOutAtR=" + scaleOutAtR, "confidence=" + confidence),
                    List.of("doflamingo", "v4", "strict", "multi-v6", "scale-out", "risk", "confidence"),
                    List.of(
                            DoflamingoSignalSupport.condition(
                                    "multi-v6-v4.scale-out-r-multiple",
                            ...
                            DoflamingoSignalSupport.condition(
                                    "multi-v6-v4.scale-out-favorable-excursion",
                            ...
                            DoflamingoSignalSupport.condition(
                                    "multi-v6-v4.scale-out-macd-nonnegative",
        ...
        longStructureWeakBars = closeBelowSpanB && !directionUp(state, current) ? longStructureWeakBars + 1 : 0;
        boolean staleExit = position.barsHeld() >= staleBars && currentR.compareTo(staleMinR) <= 0;
        boolean structureBreak = longStructureWeakBars >= structureExitConfirmBars;
        boolean timeStop = position.barsHeld() >= maxHoldingBars;
        ...
        boolean exit = reversalConfirmed || staleExit || structureBreak || timeStop || postScaleWeakness;
        if (!exit) {
            return StrategyIntentResult.empty();
        }

        BigDecimal confidence = exitConfidence(position, currentR, state, reversalConfirmed, staleExit, structureBreak || timeStop, postScaleWeakness);
        if (allowReversal && reversalConfirmed) {
            ...
            StrategyTradeIntent reverseIntent = DoflamingoSignalSupport.reverseLongToShortIntent(
                    ...
                    SetupType.REVERSAL,
                    riskFraction,
                    DoflamingoSignalSupport.percentStopWithRrTarget(stop.selectedStopPct(), targetRMultiple, "Doflamingo Multi V6 v4 short " + stopMode + " runtime stop"),
                    maxHoldingBars,
                    "Doflamingo Multi V6 v4 long-to-short reversal",
                    ...
                    List.of("doflamingo", "v4", "strict", "multi-v6", "reversal", "short", "risk", "confidence"),
                    List.of(
                            DoflamingoSignalSupport.condition("multi-v6-v4.reverse-long-to-short-enabled", "Reversal is enabled", "Allow reversal", allowReversal ? 1.0d : 0.0d, "=", "Required", 1.0d, allowReversal),
                            DoflamingoSignalSupport.condition("multi-v6-v4.reverse-long-to-short-confirmed", "Bearish reversal confirmation", "Bearish reversal", reversalConfirmed ? 1.0d : 0.0d, "=", "Required", 1.0d, reversalConfirmed)
                    )
            );
        ...
        StrategyTradeIntent exitIntent = DoflamingoSignalSupport.longExitIntent(
                ...
                confidence,
                SetupType.REVERSAL,
                "Doflamingo Multi V6 v4 lifecycle exit",
                List.of(
                ...
                List.of("doflamingo", "v4", "strict", "multi-v6", "exit", "risk", "confidence"),
                List.of(
                        DoflamingoSignalSupport.condition("multi-v6-v4.exit-reversal-confirmed", "Reversal confirmation", "Reversal confirmed", reversalConfirmed ? 1.0d : 0.0d, "=", "Required", 1.0d, reversalConfirmed),
                        DoflamingoSignalSupport.condition("multi-v6-v4.exit-stale-bars", "Stale trade bars held", "Bars held", position.barsHeld(), ">=", "Stale bars", staleBars, staleExit),
                        DoflamingoSignalSupport.condition("multi-v6-v4.exit-stale-r", "Stale trade R multiple below threshold", "Current R", currentR, "<=", "Stale minimum R", staleMinR, staleExit),
                        DoflamingoSignalSupport.condition("multi-v6-v4.exit-structure-break", "Structure broke below Span B with weak PSAR", "Structure weak bars", longStructureWeakBars, ">=", "Required bars", structureExitConfirmBars, structureBreak),
                        DoflamingoSignalSupport.condition("multi-v6-v4.exit-time-stop", "Position reached explicit max-holding time stop", "Bars held", position.barsHeld(), ">=", "Max holding bars", maxHoldingBars, timeStop),
                        DoflamingoSignalSupport.condition("multi-v6-v4.exit-trail-after-scale-out-enabled", "Trailing after scale-out is enabled", "Trail after scale-out", trailAfterScaleOut ? 1.0d : 0.0d, "=", "Required", 1.0d, trailAfterScaleOut),
                        DoflamingoSignalSupport.condition("multi-v6-v4.exit-post-scale-weakness", "Post-scale trailing weakness", "MACD histogram", state.macdHistogram(), "<", "Previous MACD histogram", state.previousMacdHistogram(), postScaleWeakness),
                        DoflamingoSignalSupport.condition("multi-v6-v4.exit-sell-stoch", "Stoch RSI sell confirmation", "Stoch sell pattern", sellSignalStoch ? 1.0d : 0.0d, "=", "Required", 1.0d, sellSignalStoch),
                        DoflamingoSignalSupport.condition("multi-v6-v4.exit-sell-macd", "MACD sell confirmation", "MACD sell pattern", sellSignalMacd ? 1.0d : 0.0d, "=", "Required", 1.0d, sellSignalMacd)
                )
        );
        cooldownRemaining = cooldownBars;
    ...
    private StrategyIntentResult shortEntryIntent(
        ...
        boolean stochDFalling = state.stochD() < state.previousStochD();
        int momentumBreadth = confirmations(macdHistogramFalling, stochKFalling, stochDFalling);
        boolean momentumBreadthOk = momentumBreadth >= 2;
        boolean momentumConfirmed = originalMomentum && momentumBreadthOk;
        ...
        boolean rsiExtremeRecent = requireRsiExtremeWithinBars == 0
                || DoflamingoIndicatorMath.recentRsiExtreme(context.instrumentHistory(), index, requireRsiExtremeWithinBars, stochOverbought, false);
        boolean volumeOk = DoflamingoIndicatorMath.volumeAtLeastAverageMultiple(context.instrumentHistory(), index, 20, volumeConfirmMultiple.doubleValue());
        if (!directionNowDown || !momentumConfirmed || !cloudConfirmed || !trendFilterPassed
                || !cleanStructure || !psarDistanceOk || !notOverextended || !rsiExtremeRecent || !volumeOk) {
            return StrategyIntentResult.empty();
        }

        BigDecimal confidence = shortConfidence(directionNowDown, sellSignalMacd, sellSignalStoch, originalMomentum,
                momentumBreadthOk, false,
        ...
        List<StrategyTradeIntentConditionEvidence> conditions = List.of(
                DoflamingoSignalSupport.condition("multi-v6-v4.short.psar-down", "PSAR confirms downward direction", "PSAR", state.psar(), ">", "Candle high", high, directionNowDown),
                DoflamingoSignalSupport.condition("multi-v6-v4.short.psar-distance", "Short PSAR distance avoids noisy flips", "PSAR distance %", psarDistancePct, ">=", "Minimum distance %", MIN_SHORT_PSAR_DISTANCE_PCT, psarDistanceOk),
                DoflamingoSignalSupport.condition("multi-v6-v4.short.macd-sell", "MACD sell confirmation", "MACD sell pattern", sellSignalMacd ? 1.0d : 0.0d, "=", "Required", 1.0d, sellSignalMacd),
                DoflamingoSignalSupport.condition("multi-v6-v4.short.stoch-sell", "Stoch RSI overbought sell confirmation", "Stoch sell pattern", sellSignalStoch ? 1.0d : 0.0d, "=", "Required", 1.0d, sellSignalStoch),
                DoflamingoSignalSupport.condition("multi-v6-v4.short.cloud-confirmation", "Price is below bearish cloud floor", shortCloudMode == ShortCloudMode.HIGH_BELOW_CLOUD ? "Candle high" : "Candle close", shortCloudMode == ShortCloudMode.HIGH_BELOW_CLOUD ? high : close, "<", "Cloud floor", cloudFloor, cloudConfirmed),
                DoflamingoSignalSupport.condition("multi-v6-v4.short.clean-structure", "Short structure is clean below the cloud", "Clean structure", cleanStructure ? 1.0d : 0.0d, "=", "Required", 1.0d, cleanStructure),
                DoflamingoSignalSupport.condition("multi-v6-v4.short.overextension", "Below-cloud distance is not overextended", "Below-cloud distance %", belowCloudDistancePct, "<=", "Max allowed %", maxStopPct.doubleValue() * 2.0d, notOverextended),
                DoflamingoSignalSupport.condition("multi-v6-v4.short.raw-stop-pct", "Raw short stop distance before bounds", "Raw stop %", stop.rawStopPct(), "<=", "Max stop %", maxStopPct, stop.rawStopPct().compareTo(maxStopPct) <= 0),
                DoflamingoSignalSupport.condition("multi-v6-v4.short.trend-filter", "Bearish trend filter passed", "Trend filter", trendFilterPassed ? 1.0d : 0.0d, "=", "Required", 1.0d, trendFilterPassed),
                DoflamingoMarketRegimeFilter.allowedCondition("multi-v6-v4.short.market-regime-allowed", context, skipMarketRegimes),
                DoflamingoSignalSupport.condition("multi-v6-v4.short.confidence-threshold", "Dynamic short confidence meets threshold", "Confidence", confidence.doubleValue(), ">=", "Minimum confidence", minConfidence.doubleValue(), true)
        );
        TradeSignal signal = DoflamingoSignalSupport.shortSignal(strategyId(),
                DoflamingoMultiIndicatorV6TrendReversalV4StrategyProvider.STRATEGY_VERSION, context, confidence, SetupType.REVERSAL);
        StrategyTradeIntent intent = DoflamingoSignalSupport.shortEntryIntent(
                ...
                DoflamingoSignalSupport.percentStopWithRrTarget(stop.selectedStopPct(), targetRMultiple, "Doflamingo Multi V6 v4 short " + stopMode + " runtime stop"),
                maxHoldingBars,
                "Doflamingo Multi V6 v4 bearish PSAR, momentum, cloud, and trend-filter reversal entry",
                List.of(
                        "side=SHORT",
                ...
                List.of("doflamingo", "v4", "strict", "multi-v6", "entry", "short", "risk", "confidence"),
                conditions
        );
        return new StrategyIntentResult(List.of(signal), List.of(intent), List.of());
    }
    ...
    private StrategyIntentResult shortPositionIntent(
        ...
        if (canScaleIn) {
            ...
            StrategyTradeIntent scaleIntent = DoflamingoSignalSupport.shortScaleInIntent(
                    ...
                    scaleOutFraction,
                    maxShortScaleIns,
                    "Doflamingo Multi V6 v4 short scale-in at configured R multiple",
                    List.of("side=SHORT", "action=SCALE_IN_SHORT", "currentR=" + currentR, "shortScaleInAtR=" + shortScaleInAtR,
                            ...
                            "cloudCeiling=" + BigDecimal.valueOf(cloudCeiling).setScale(4, RoundingMode.HALF_UP),
                            "confidence=" + confidence),
                    List.of("doflamingo", "v4", "strict", "multi-v6", "scale-in", "short", "risk", "confidence"),
                    List.of(
                            DoflamingoSignalSupport.condition("multi-v6-v4.short.scale-in-enabled", "Short scale-in is enabled", "Allow short scale-in", allowShortScaleIn ? 1.0d : 0.0d, "=", "Required", 1.0d, allowShortScaleIn),
                            DoflamingoSignalSupport.condition("multi-v6-v4.short.scale-in-r-multiple", "Current R multiple reached short scale-in threshold", "Current R", currentR, ">=", "Scale-in R", shortScaleInAtR, true),
                            DoflamingoSignalSupport.condition("multi-v6-v4.short.scale-in-count", "Short scale-in count is below maximum", "Scale-in count", position.scaleInCount(), "<", "Max short scale-ins", maxShortScaleIns, position.scaleInCount() < maxShortScaleIns),
                            DoflamingoSignalSupport.condition("multi-v6-v4.short.scale-in-momentum", "Renewed bearish momentum is present", "Renewed bearish momentum", renewedBearishMomentum ? 1.0d : 0.0d, "=", "Required", 1.0d, renewedBearishMomentum),
                            DoflamingoSignalSupport.condition("multi-v6-v4.short.scale-in-clean-structure", "Short structure remains clean", "Clean structure", cleanStructure ? 1.0d : 0.0d, "=", "Required", 1.0d, cleanStructure)
                    )
            );
            return new StrategyIntentResult(List.of(), List.of(scaleIntent), List.of());
        }
        ...
        if (canScaleOut) {
            ...
            StrategyTradeIntent scaleIntent = DoflamingoSignalSupport.shortScaleOutIntent(
                    ...
                    SetupType.REVERSAL,
                    scaleOutFraction,
                    "Doflamingo Multi V6 v4 short scale-out at configured R multiple",
                    ...
                    List.of("doflamingo", "v4", "strict", "multi-v6", "scale-out", "short", "risk", "confidence"),
                    List.of(
                            DoflamingoSignalSupport.condition("multi-v6-v4.short.scale-out-r-multiple", "Current R multiple reached scale-out threshold", "Current R", currentR, ">=", "Scale-out R", scaleOutAtR, true),
                            DoflamingoSignalSupport.condition("multi-v6-v4.short.scale-out-favorable-excursion", "Short has favorable excursion", "Max favorable %", position.maxFavorablePct(), ">", "Zero", BigDecimal.ZERO, position.maxFavorablePct().signum() > 0),
                            DoflamingoSignalSupport.condition("multi-v6-v4.short.scale-out-thesis-valid", "Short thesis remains valid", "Short thesis valid", shortThesisValid ? 1.0d : 0.0d, "=", "Required", 1.0d, shortThesisValid)
                    )
            );
            return new StrategyIntentResult(List.of(), List.of(scaleIntent), List.of());
        }
        ...
        if (allowReversal && bullishReversalConfirmed) {
            StrategyTradeIntent reverseIntent = DoflamingoSignalSupport.reverseShortToLongIntent(
                    ...
                    maxHoldingBars,
                    "Doflamingo Multi V6 v4 short-to-long reversal",
                    List.of("side=LONG", "previousSide=SHORT", "allowReversal=" + allowReversal,
                            ...
                            "positionBarsHeld=" + position.barsHeld(), "confidence=" + confidence),
                    List.of("doflamingo", "v4", "strict", "multi-v6", "reversal", "long", "risk", "confidence"),
                    List.of(
                            DoflamingoSignalSupport.condition("multi-v6-v4.reverse-short-to-long-enabled", "Reversal is enabled", "Allow reversal", allowReversal ? 1.0d : 0.0d, "=", "Required", 1.0d, allowReversal),
                            DoflamingoSignalSupport.condition("multi-v6-v4.reverse-short-to-long-confirmed", "Bullish reversal confirmation", "Bullish reversal", bullishReversalConfirmed ? 1.0d : 0.0d, "=", "Required", 1.0d, bullishReversalConfirmed)
                    )
            );
            return new StrategyIntentResult(List.of(), List.of(reverseIntent), List.of());
        }
        StrategyTradeIntent exitIntent = DoflamingoSignalSupport.shortExitIntent(
                ...
                confidence,
                SetupType.REVERSAL,
                "Doflamingo Multi V6 v4 short lifecycle exit",
                ...
                List.of("doflamingo", "v4", "strict", "multi-v6", "exit", "short", "risk", "confidence"),
                List.of(
                        DoflamingoSignalSupport.condition("multi-v6-v4.short.exit-bullish-reversal", "Bullish reversal confirmation", "Bullish reversal", bullishReversalConfirmed ? 1.0d : 0.0d, "=", "Required", 1.0d, bullishReversalConfirmed),
                        DoflamingoSignalSupport.condition("multi-v6-v4.short.exit-stale-bars", "Stale short bars held", "Bars held", position.barsHeld(), ">=", "Stale bars", staleBars, staleExit),
                        DoflamingoSignalSupport.condition("multi-v6-v4.short.exit-stale-r", "Stale short R multiple below threshold", "Current R", currentR, "<=", "Stale minimum R", staleMinR, staleExit),
                        DoflamingoSignalSupport.condition("multi-v6-v4.short.exit-structure-break", "Short structure broke above cloud ceiling with weak PSAR", "Candle close", close, ">", "Cloud ceiling", cloudCeiling, structureBreak),
                        DoflamingoSignalSupport.condition("multi-v6-v4.short.exit-post-scale-recovery", "Post-scale short trailing recovery", "MACD histogram", state.macdHistogram(), ">", "Previous MACD histogram", state.previousMacdHistogram(), postScaleRecovery),
                        DoflamingoSignalSupport.condition("multi-v6-v4.short.exit-buy-stoch", "Stoch RSI buy confirmation", "Stoch buy pattern", buySignalStoch ? 1.0d : 0.0d, "=", "Required", 1.0d, buySignalStoch),
                        DoflamingoSignalSupport.condition("multi-v6-v4.short.exit-buy-macd", "MACD buy confirmation", "MACD buy pattern", buySignalMacd ? 1.0d : 0.0d, "=", "Required", 1.0d, buySignalMacd)
                )
        );
        return new StrategyIntentResult(List.of(), List.of(exitIntent), List.of());
    }
    ...
    private boolean trendFilterPassed(
        ...
        return aboveEma50 || improvingTrend || reclaimWithMacdConfirmation;
    }
    ...
    private boolean shortTrendFilterPassed(
        ...
        return belowEma50 || weakeningTrend || breakdownWithMacdConfirmation;
    }
    ...
    private TradeIntentExitPolicy stopPolicy(double close, double presentSpanB, OptionalDouble maybeAtr) {
        ...
        return DoflamingoSignalSupport.percentStopWithRrTarget(
                BigDecimal.valueOf(selected).setScale(4, RoundingMode.HALF_UP),
                targetRMultiple,
                "Doflamingo Multi V6 v4 " + stopMode + " runtime stop"
        );
    }
    ...
    private TradeIntentExitPolicy shortStopPolicy(double close, double cloudCeiling, OptionalDouble maybeAtr) {
        ...
        return DoflamingoSignalSupport.percentStopWithRrTarget(
                ...
                targetRMultiple,
                "Doflamingo Multi V6 v4 short " + stopMode + " runtime stop"
        );
    }
    ...
    private BigDecimal confidence(
            ...
            boolean momentumBreadthConfirmed,
            boolean unusedAdaptiveMomentumUsed,
            double close,
            DoflamingoIndicatorMath.MultiIndicatorState state,
            OptionalDouble maybeEma50,
            OptionalDouble maybeTrend,
        ...
        if (originalMomentum && momentumBreadthConfirmed) {
            score += 0.02d;
        }
    ...
    private boolean portfolioDrawdownBlocked(StrategyExecutionContext context) {
        return maxPortfolioDrawdownPct.signum() > 0
                && context.portfolioState().maxDrawdownPctSoFar().abs().compareTo(maxPortfolioDrawdownPct) >= 0;
    }

    private static int confirmations(boolean first, boolean second, boolean third) {
        int count = 0;
        if (first) {
            count++;
        }
